...
The CSV file we're going to read is a list of all calls to UNLPD since 2018.
...
Let's look at this. As you can see by the data, we have six players, **which is what we expect** from `head()`. But notice the first row -- the headers. We have things like `incident_code` and `reported` for what type of call it is and the date and time it was reported. 
