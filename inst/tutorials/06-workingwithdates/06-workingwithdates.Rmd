...
How about what month are the most calls? We could use formatting to create a Month field but that would group all the Aprils together, regardless of if they were in 2022 or 2021. We could create a year and a month together, but that would give us an invalid date object and that would create problems later. Lubridate has something called a floor date that we can use. 

We're going to use mutate to create a floor_month field. Keep this code handy -- you'll need it for the homework.  

```{r floormonth, exercise=TRUE, exercise.setup = "load-data"}
calls %>% 
  mutate(
    clean_reported = ???_??(reported),
    floor_month = floor_date(?????_????????, "month")
    ) 
```
