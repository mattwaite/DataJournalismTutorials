...
pop21 <- counties21 %>% select(GEOID, NAME, estimate) %>% rename(Population20??=estimate)

pop11 <- counties11 %>% select(GEOID, NAME, estimate) %>% rename(Population20??=estimate)
```
...
alldata %>% 
  mutate(PercentChange = (Population20?? - Population20??)/Population20??) %>%
