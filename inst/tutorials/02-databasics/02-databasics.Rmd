...
The <code>\<-</code> is what creates the name. The name is to the left. What goes in the name is on the right.
...
The name of the file goes inside <code>read_csv</code> in quotes.
...
<code>head()</code> only shows you the first six rows.
...
![](images/napkin-headvsglimpse.png){width="100%"}
...
So the names of every call type in this dataset are in the incident_code column. Want to know what call is the most common on campus? You'll need the incident_code column and a way to count each one of them. Looking at that first row for those names is something you're going to do over and over again.
...
![](images/napkin-types.png){width="100%"}
